...
with gr.Blocks(title="Avatar Compositor") as app:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            character_rotation = gr.Slider(-10, 10, value=3.0, step=0.5, label="Rotation (degrees, + = counter-clockwise)")
            face_position = gr.Slider(0.0, 1.0, value=0.0, step=0.05, label="Face Position (0=top, 1=bottom)")

            gr.Markdown("### Character Offset")
            character_x_offset = gr.Slider(-100, 100, value=-70, step=1, label="X Offset")
            character_y_offset = gr.Slider(-200, 200, value=-40, step=1, label="Y Offset")

            gr.Markdown("### Mask Offset")
            mask_x_offset = gr.Slider(-50, 100, value=0, step=1, label="Mask X")
            mask_y_offset = gr.Slider(-200, 50, value=0, step=1, label="Mask Y")

            gr.Markdown("### Portal Offset")
            portal_x_offset = gr.Slider(-50, 100, value=0, step=1, label="Portal X")
            portal_y_offset = gr.Slider(-50, 100, value=0, step=1, label="Portal Y")
